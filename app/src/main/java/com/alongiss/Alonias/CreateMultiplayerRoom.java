...
public class CreateMultiplayerRoom extends AppCompatActivity {
    ...
    private void spinnerUpdate() {
        ...
        playersAdapter.setDropDownViewResource(R.layout.spinner_dropdown_item_white);
            ...
            public void handleMessage(@NonNull Message msg) {
                ...
                if (response.startsWith("crt~")) {
                    ...
                    if ("True".equals(p[1]) && p.length >= 3) {
                        ...
                        Toast.makeText(CreateMultiplayerRoom.this, ClientMessageUtils.roomCreateSuccessMessage(), Toast.LENGTH_SHORT).show();
    ...
    private boolean validateInputs(String roomName, String password) {
        if (roomName.length() < 3 || roomName.length() > 10) {
            toast("Room name must be 3 to 10 characters long.");
            return false;
        }

        if (password.length() < 3 || password.length() > 10) {
            toast("Password must be 3 to 10 characters long.");
            return false;
        }
